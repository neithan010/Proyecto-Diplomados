...
where
	...
	CONCAT(p.etapa, p.estado) IN ('2030') 
...
$objPHPExcel->getProperties()->setCreator("Admisión - Unegocios - Centro de Desarrollo Gerencial")
							 ->setLastModifiedBy("Docencia CDG")
							 ->setTitle("BBDD Rechazados")
...
// Add some data
$objPHPExcel->getActiveSheet()->SetCellValue('D2','Rechazadas');
...
$objPHPExcel->setActiveSheetIndex(0)
			...
			->setCellValue('M'.$fila, 'OBSERVACIÓN')
			->setCellValue('N'.$fila, 'MATRICULADO OTRO PROGRAMA')
			;
...
$objPHPExcel->getActiveSheet()->getStyle('A1:N1')->getFill()->getStartColor()->setARGB('FFC0C0C0');
...
while ($row_rs_matriculados = $stmt_pre_post->fetch()){
	...
	$objPHPExcel->setActiveSheetIndex(0)
			...
			->setCellValue('M'.$cont, str_replace('<br>','',utf8_encode($row_rs_matriculados['observaciones'])))
			->setCellValue('N'.$cont, utf8_encode($row_rs_matriculados['otro_programa']))
			;
...
// Rename worksheet
$objPHPExcel->getActiveSheet()->setTitle('Rechazadas');
...
header('Content-Disposition: attachment;filename="Rechazadas '.$cod_diploma.'.xls"');
