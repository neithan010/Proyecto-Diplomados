...
while ($rws = $resultado->fetch()){
    $arr_packageID[]=$rws['packageID'];
    $arr_documento[]=$rws['documento'];
    $email=$rws['EMAIL'];
...
$i=0;
foreach($arr_packageID as $packageID){


    //$callback_url='https://intranet.unegocios.cl/apps/certinet/';
    $callback_url='https://intranet.unegocios.cl/apps/admision_2021_test/dashboard/dist/certinet/firma_exito.php';
...
if($data['error']=='false'){
    echo '<a href="'.$data['integration_url'].'">link Firma '.$arr_documento[$i].'</a>';
}else{
    echo 'Error: '.$data['mensaje'].' :: '.$data['detalles'].'<br>';
}
