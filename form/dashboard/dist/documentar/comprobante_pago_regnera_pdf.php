...
ORDER BY cp.idcomprobante_pago";
...
if ($rw_data  = $stmt_postulante ->fetch()){
	...
	$idcomprobante_pago     = $rw_data['idcomprobante_pago'];
    $n                      = $rw_data['n'];
    $facturar_a             = $rw_data['facturar_a'];
	$rut                    = $rw_data['rut'];
    $direccion              = utf8_encode($rw_data['direccion']);
	$participante           = utf8_encode($rw_data['participante']);
    $orden_compra           = $rw_data['orden_compra'];
    $facturar_a_emp         = utf8_encode($rw_data['facturar_a_emp']);
    $fecha_facturacion_emp  = $rw_data['fecha_facturacion_emp'];
    $rut_emp                = $rw_data['rut_emp'];
    $direccion_emp          = utf8_encode($rw_data['direccion_emp']);
...
if($nombre_empresa<>'' && ($qpaga=='Empresa' || $qpaga=='Interesado/Empresa')){
    $pdf->Cell(80,5,': '.utf8_decode($facturar_a_emp),'0',0,'L');
}else{
...
if($qpaga=='Empresa' || $qpaga=='Interesado/Empresa'){   
    ...
    $pdf->Cell(80,5,': '.$direccion_emp,'0',0,'L');
}else{
...
if(isset($orden_compra)){
    $text_oc=$orden_compra;
}
        

        if($qpaga=='Interesado' || $qpaga=='Interesado/Empresa'){
            $pdf->Ln(2);
            $pdf->SetFont('Arial','B',8);
