...
if ($row_rs_datos = $stmt_data->fetch()){
	...
	$diploma=utf8_encode($row_rs_datos['POSTULACION']);
	$programa=utf8_encode($row_rs_datos['POSTULACION']);
	...
	$mes_inicio=mes_es($row_rs_datos['mes_inicio']);
...
<input id="email" name="email" type="hidden" value="<?php echo $row_rs_datos['email'];?>"/>
<input id="cod_diploma" name="cod_diploma" type="hidden" value="<?php echo $row_rs_datos['cod_diploma'];?>"/>
...
<h2 class=" text-center">CONTRATO DE PRESTACI&Oacute;N DE SERVICIOS EDUCACIONALES</h2>
<div class="interlineado">
<p>
<p>En Santiago, a <input id="fecha_imp" name="fecha_imp" type="text" value="<?php echo $fecha;?>"/>, entre la FACULTAD DE ECONOMÍA Y NEGOCIOS DE LA UNIVERSIDAD DE CHILE, rol único tributario número 60.910.000-1, y don(ña) <input id="nombre_completo" name="nombre_completo" type="text" value="<?php echo $nombre_completo;?>" size="<?php echo strlen($nombre_completo)+5;?>" />, de nacionalidad <input id="NACIONALIDAD" name="NACIONALIDAD" type="text" value="<?php echo utf8_encode($row_rs_datos['NACIONALIDAD']);?>" />, cédula de identidad número <input id="RUT" name="RUT" type="text" value="<?php echo $rut;?>" />, domiciliado(a) <input id="DIREC_PARTICULAR" name="DIREC_PARTICULAR" type="text" value="<?php echo $direccion;?>" size="<?php echo strlen($direccion)+5;?>" />, en adelante indistintamente el “Alumno”, se ha convenido el presente Contrato de Prestación de Servicios Educacionales:</p>
<p>PRIMERO: El Alumno ha sido  aceptado y matriculado en el <strong>&quot;<?php echo $programa;?>&quot;</strong><input type="hidden" name="diploma" id="diploma" value="<?php echo $programa;?>"  />, impartido por la  Unidad de Extensión del Departamento de Administración de la Facultad de  Economía y Negocios de la Universidad de Chile. </p>
<p>Dicho programa se inicia  en el mes de <input name="ini_programa" id="ini_programa" type="text" value="<?php echo strtoupper($mes_inicio)?>" /> de <input name="year_inicio" id="year_inicio" type="text" value="<?php echo $year_inicio;?>" />
 y se encuentra regido, en general, por el Decreto Exento Nº 0011180, de fecha 23 de abril de 2020, y en especial,  por el Reglamento para Diplomas de Extensión, el Reglamento para Cursos y los  Términos y Condiciones de Programas, todos documentos disponibles en el sitio  web &lt;https://unegocios.uchile.cl/reglamento/&gt;. El Alumno  declara conocer, aceptar y acatar cabalmente y en todas sus partes dichos reglamentos  y demás normativas que se encuentren vigentes durante el desarrollo del  Diploma, las que para todos los efectos forman parte integrante del presente  contrato.</p>
<p>SEGUNDO: El Alumno acepta  las políticas para la dictación de los diplomas que se señalan a continuación:</p>
<p>a) Existirá un Comité Académico que fijará, discrecionalmente las políticas relativas a: contenidos, evaluación, duración del Diploma, así como la asignación de los profesores y selección de alumnos y, en general, todas las políticas globales del funcionamiento de cada Diploma.</p>
<p>b) El alumno que postule a un Diploma deberá ser previamente aceptado por la Unidad de Extensión. La aceptación de un postulante a cualquier Diploma dependerá del cumplimiento de los distintos requisitos que para cada programa se exijan, tales como conocimientos previos, experiencia laboral, títulos profesionales o técnicos y cualquier otro que la Unidad de Extensión, a su solo arbitrio, estime prudente para un buen desempeño tanto del alumno, como para el enriquecimiento del Diploma..</p>
<p>c) Por situaciones de caso fortuito o fuerza mayor, o por razones académicas y/o administrativas de mejor servicio y calidad de los programas, la Unidad de Extensión podrá modificar los contenidos e información del Diploma, tales como su malla curricular, horarios, días de clases, profesores, relatores y modalidad de las clases. Desde ya, el Alumno acepta las eventuales modificaciones que puedan realizarse al Diploma como una medida necesaria para el desarrollo del programa, por lo que se exime a la Unidad de Extensión, al Departamento de Administración, a la Facultad de Economía y Negocios y a la Universidad de Chile, de todo tipo de responsabilidad por los daños y perjuicios de cualquier naturaleza que se puedan producir debido a cambios o modificaciones en los Diplomas, no procediendo por ello, en caso alguno, pago de ningún tipo de indemnización por este motivo.</p>
<p>d) Asimismo, la Unidad de Extensión podrá decidir suspender o bien no dictar algún Diploma, por carecer del mínimo de alumnos inscritos o matriculados necesarios para su dictación. El número de alumnos necesario para cada Curso o Diploma es fijado en forma discrecional por el Comité Académico de cada uno de ellos. En este caso, la Unidad de Extensión informará mediante cualquier forma idónea al Alumno, indicándole la causa de la suspensión o no dictación del Diplomado. El Alumno exime a la Unidad de Extensión, al Departamento de Administración, a la Facultad de Economía y Negocios y a la Universidad de Chile, de todo tipo de responsabilidad por los daños y perjuicios de cualquier naturaleza que se puedan producir debido a la suspensión y/o no dictación del Diploma, no procediendo, en caso alguno, pago de ningún tipo de indemnización por este motivo.</p>
<p>e) La Unidad de Extensión pondrá a disposición de los alumnos una plataforma online en la cual encontrarán para su descarga y uso exclusivo, todo el material de clases relevante para el proceso de aprendizaje. Esto en el marco del Acuerdo de Producción Limpia firmado por el ex Rector de la Universidad, don Víctor Pérez, e impulsado en nuestra Facultad de Economía y Negocios por EcoFEN. Así, se busca reducir al mínimo y, en lo posible, eliminar por completo, las impresiones de documentos, bibliografía, casos y otros.</p>
<p>TERCERO: Uno. El precio total del Diploma asciende a la suma  <input id="valor_programa" name="valor_programa" type="text" value="<?php echo number_format($row_rs_datos['valor_diplomado'], 0, ",", ".");?>" size="10" maxlength="8" /> 
	<?php if($row_rs_datos['moneda']=='CLP'){ 
		echo 'PESOS</strong>';
	?><input type="hidden" name="moneda" id="moneda" value="PESOS" />
	<?php 
	}else{ 
		echo 'Unidades de Fomento</strong>, en su equivalente en pesos a la fecha efectiva de pago';
		?><input type="hidden" name="moneda" id="moneda" value="Unidades de Fomento, en su equivalente en pesos a la fecha efectiva de pago" /><?php 
	}?>, menos los descuentos eventuales que procedan en conformidad a lo establecido en la Resolución Exenta N° 197.20 y a la REX N° 302.20 que la modifica, y se señalen expresamente en el anexo denominado “Control Cumplimiento Decretos y Condiciones de Cierre de Matrícula” (en adelante, el “Anexo”), el que debidamente firmado por el Alumno es parte integrante del presente contrato para todos los efectos.<br />
  Con la finalidad de facilitar el pago en cuotas, y sin que ello implique novación de ningún tipo, se autoriza al Alumno a efectuar el pago correspondiente a través de los siguientes medios:</p>
<p>1. Entrega a la  Universidad de tantos cheques como cuotas hayan sido pactadas, los cuales se  individualizarán en el Anexo, con un máximo de 12 documentos.<br>
   2. Tarjeta de Crédito Bancarias (Hasta 18 cuotas sin interés, tarjeta Falabella hasta 12 cuotas). <br>
   3. PayPal.<br>
   4. PAT con un máximo de 12 cuotas.</p>
...
<p>2)  Cobrar en forma directa o a través de una empresa de cobranza externa que se informará al alumno, los gastos de cobranza extrajudicial sobre las deudas morosas o atrasadas de acuerdo a lo establecido en el artículo 37 de la citada Ley No 19.496. El monto por concepto de cobranza no podrá superar los siguientes porcentajes aplicados sobre el monto de la deuda vencida a la fecha del atraso, y conforme a la siguiente escala progresiva:</p>
<ol>
...
<p>Por último, si el alumno pacta cuotas ya sea en sistema WebPay o PAT, estas cuotas deberán estar canceladas en su totalidad al día de la ceremonia de clausura del programa, de lo contrario quedará pendiente la entrega del diplomado para el siguiente período de ceremonias. El alumno podrá prepagar las cuotas pendientes al día indicado de la ceremonia si así lo decide. </p>
<p>Dos. El precio del Diploma será pagado de la siguiente manera (marcar sólo el cuadro que corresponda):</p>
<table border="1" cellspacing="0" cellpadding="0">
  <tr>
    <td width="47" valign="top"><p><input type="radio" name="quien_paga" id="alumno" value="alumno"></p></td>
    <td width="542" valign="top"><p>Por el alumno en su totalidad.</p></td>
  </tr>
  <tr>
    <td width="47" valign="top"><p><input type="radio" name="quien_paga" id="su_empresa" value="su_empresa"></p></td>
  ...
  <tr>
    <td width="47" valign="top"><p><input type="radio" name="quien_paga" id="alumno_empresa" value="alumno_empresa"></p></td>
...
<p>Lo señalado precedentemente es aplicable exclusivamente para el caso de retiro del Alumno en forma voluntaria, el que deberá efectuarse en la forma indicada.</p>
<p>Dos. En caso de retiro voluntario, reprobación y/o expulsión del Alumno del Diploma, éste perderá todo beneficio o derecho a los que hubiere accedido por haber poseído la calidad de alumno de la Universidad, tales como invitaciones a conferencias, charlas, membresías, suscripciones a revistas, etcétera.</p>
<p>Tres. El Alumno podrá realizar la postergación del Diploma, según lo establecido en el Reglamento para Diplomas de Extensión del Departamento de Administración, pudiendo reintegrarse hasta dentro de dos semestres consecutivos posteriores, siempre y cuando el programa esté disponible dentro de la oferta.. Para hacer uso de este requerimiento, se debe enviar una carta dirigida al Director de la Unidad de Extensión del Departamento de Administración de la Facultad de Economía y Negocios de la Universidad de Chile, adjuntando los respaldos necesarios para ello (certificados médicos, carta de empresa sobre traslados, finiquito, entre otros casos). En este caso, el Director de la Unidad emitirá un pronunciamiento fundado, aceptando o rechazando la postergación, sin que ello dé derecho a reclamo alguno por parte del Alumno. </p>
<p>Si la postergación del Diploma es aceptada, la Facultad podrá otorgar al alumno la exención de un porcentaje del arancel en los términos establecidos en el número uno precedente, según lo dispuesto en el artículo 1° numeral XI de la Resolución Exenta N° 197.20, modificada por la REX N° 302.20. En todo caso, una vez que el alumno se reincorpore a la nueva versión del diploma, deberá pagar a la Universidad el arancel de reincorporación cuyo monto será proporcional al tiempo que le falte para terminar el programa, más un 1% del valor del arancel del diploma por concepto de gastos de administración. En el caso que la nueva versión del diploma tenga un arancel superior al diploma postergado, el participante deberá pagar, adicionalmente, la diferencia de valor.</p>
<p>QUINTO: Las partes acuerdan que en caso de fuerza mayor o caso fortuito, que haga imposible la dictación de todo o parte del Diploma en las dependencias de la Unidad de Extensión en formato presencial, éste podrá ser impartido en otras sedes, lugares o bien en forma virtual, mediante plataforma electrónica o vía internet, ello a criterio exclusivo de la Universidad y la Unidad de Extensión. Esto no afectará la certificación del programa, en cuanto a horas y desarrollo de contenidos. </p>
...
<table class="firma" width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td width="50%" align="center"><strong><?php echo strtoupper($nombre_decano)?></strong></td>
    <td width="50%" align="center">&nbsp;</td>
  </tr>
  <tr>
    <td width="50%" align="center"><?php echo strtoupper($titulo_decano)?></td>
    <td width="50%" align="center">ALUMNO</td>
  </tr>
  <tr>
    <td align="center">FACULTAD DE ECONOMÍA Y  NEGOCIOS UNIVERSIDAD DE CHILE</td>
    <td align="center">&nbsp;</td>
  </tr>
</table>
</form>
...
<p align="center">
	<button class="btn btn-primary" id="btn_imp_fd" >Firma Digital</button> 
	<button class="btn btn-primary" id="btn_imp_pdf">Firma Insitu</button> 
...
$( document ).ready(function() {
    ...
    $("#btn_imp_pdf" ).click(function(){
           $("#form1").attr('action','texto/contrato_prestacion_servicio_pdf.php');
    ...
    $("#btn_imp_fd" ).click(function(){
		...
		$.post( "../../../../cdg/postulacion/admision/g_documentos/contratos/contrato_prestacion_servicio_firma_digital.php", 
		{ 
			...
			$.post( "../certinet/crear_contrato.php", 
			{ 
